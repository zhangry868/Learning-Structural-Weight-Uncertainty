...
class VPG_multi_Stein(BatchPolopt, Serializable):
    def __init__(
        ...
        self.stein_learning_rate_decay = 1
    ...
    def train(self):
        ...
        for itr in range(self.current_itr, self.n_itr):
            ...
            with logger.prefix('itr #%d | ' % itr):
                average_return_list = []
                gradient_list = []
                if((itr % 100 == 0) and (itr > 100)):
                     self.stein_learning_rate_decay = self.stein_learning_rate_decay * 0.5
                for i in range(self.num_of_agents):
    ...
    def update_policies(self, gradient_list):
        ...
        w2_distance_matrix = np.sum(np.square(self.target_params[None, :, :] - params[:, None, :]), axis=-1)
        
        w2_distance_vector = w2_distance_matrix.flatten()
        w2_distance_vector.sort()
        w2_median = 0.5 * (w2_distance_vector[int(len(w2_distance_vector) / 2)] + w2_distance_vector[int(len(w2_distance_vector) / 2) - 1])
        w2_h = w2_median / (2 * np.log(self.num_of_agents + 1))

        self.w2_distance_vector = np.sum(np.multiply(1-w2_distance_matrix/w2_h, np.exp(-w2_distance_matrix/w2_h)), axis=1)
        grad_w2 = np.multiply(self.w2_distance_vector[:, None], params - self.target_params) / self.num_of_agents
        ...
        weights = -np.mean(weights[:, :, :], axis=0)  + self.w2_scale * grad_w2

        self.target_params = np.copy(params)
        ...
        if self.stein_optimization_method == 'adam':
            ...
            params = params - self.stein_learning_rate * self.stein_learning_rate_decay * (m_hat / (np.sqrt(v_hat) + self.stein_epsilon))
        elif self.stein_optimization_method == 'adagrad':
            if self.stein_m is None:
                self.stein_m = np.zeros_like(params)
            self.stein_m = self.stein_m + np.square(weights)
            params = params - self.stein_learning_rate * self.stein_learning_rate_decay * (weights / (np.sqrt(self.stein_m + self.stein_epsilon)))
    ...
    def init_opt(self):
        ...
        if(self.target_params is None):
            self.target_params = np.copy(np.array([self.policy_list[i].get_param_values() for i in range(self.num_of_agents)]))
